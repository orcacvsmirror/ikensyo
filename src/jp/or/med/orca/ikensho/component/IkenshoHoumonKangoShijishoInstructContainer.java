...
public class IkenshoHoumonKangoShijishoInstructContainer extends VRPanel {
    ...
    private IkenshoTextFieldDocumentLimit text = new IkenshoTextFieldDocumentLimit();
    ...
    // [ID:0000514][Tozo TANAKA] 2009/09/09 replace end
    // 【2009年度対応：訪問看護指示書】特別指示書の管理機能
    ...
    // [ID:0000514][Tozo TANAKA] 2009/09/09 add begin
    // 【2009年度対応：訪問看護指示書】特別指示書の管理機能
    ...
    public IkenshoHoumonKangoShijishoInstructContainer() {
        ...
        showSelect.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                // [ID:0000514][Tozo TANAKA] 2009/09/24 replace begin
                // 【2009年度対応：訪問看護指示書】特別指示書の管理機能
                // setText(new IkenshoExtraSpecialNoteDialog(getCaption(),
                // getCode(),
                // getMaxLength(),
                // text.getColumns(),
                // text.getRows(),
                ...
                // getText()));
                setText(new IkenshoExtraSpecialNoteDialog(getCaption(),
                        getCode(), getMaxLength(), text.getColumns(), text
                                .getMaxRows(), teikeiMaxLength)
                        .showModal(getText()));
                // [ID:0000514][Tozo TANAKA] 2009/09/24 replace end
                // 【2009年度対応：訪問看護指示書】特別指示書の管理機能
            }
        });

        check.addItemListener(new ACFollowDisabledItemListener(
                new JComponent[] { showSelect, text }));
    ...
    public void setLoadRecentSetting(ACDBManager dbm, String patientNo,
        ...
        getLoadRecent().setLoadRecentSetting(dbm, patientNo, fieldName,
                formatKbn, text, mnemonic, message);
        getLoadRecent().setVisible(visible);
    }
     ...
     * 帳票2枚印字にする際の設定を行います。
     * 
     * @param limitLength 制限文字数
     * @param limitRows 制限行数
     * [ID:0000634][Masahiko.Higuchi] add【2011年度対応：訪問看護指示書】帳票印字文字数の拡大
     */
    public void setPageBreakLimitProperty(int limitLength, int limitRows) {
        text.setPageBreakLimitLength(limitLength);
        text.setPageBreakLimitRow(limitRows);
    }
    
    /**
     * 改ページが必要であるか判定
     * @return true:改ページ必要 false:改ページ不要
     * [ID:0000635][Shin Fujihara] add 【2010年度要望対応】
     *
     */
    public boolean isPageBreak() {
    	if (check.isVisible() && !check.isSelected()) {
    		return false;
    	}
    	return text.isPageBreak();
    }

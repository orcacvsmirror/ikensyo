...
//                        null) }));
                new ACTableColumn(7, 30, "　",
                        new IkenshoCheckBoxTableCellRenderer(),
                        outputCheckEditor),
                new ACTableColumn(8, 60, "取消", IkenshoConstants.FORMAT_CANCEL,
                        tableCellRenderer, null),
                new ACTableColumn(0, 180, "医師氏名", tableCellRenderer, null),
                new ACTableColumn(1, 180, "患者氏名", tableCellRenderer, null),
                new ACTableColumn(2, 60, "性別", SwingConstants.CENTER,
                        IkenshoConstants.FORMAT_SEX, tableCellRenderer, null),
                new ACTableColumn(3, 60, "年齢", SwingConstants.RIGHT,
                        IkenshoConstants.FORMAT_NOW_AGE, tableCellRenderer,
                        null),
                new ACTableColumn(4, 320, "被保険者番号／受給者番号", tableCellRenderer,
                        null),
                new ACTableColumn(5, 220, "作成依頼日",
                        IkenshoConstants.FORMAT_ERA_YMD, tableCellRenderer,
                        null),
                new ACTableColumn(6, 220, "意見書記入日",
                        IkenshoConstants.FORMAT_ERA_YMD, tableCellRenderer,
                        null) }));
...
//                    + (getDataString(map, "IKN_TOKKI").replaceAll("\r\n",VT)).replaceAll("\n", VT));
    ...
    public boolean doOutputCSVIshi(VRArrayList data, File file)
		...
		for (int i = 0; i < max; i++) {
			...
			// 120:行動上の障害の有無‐有無
			sbBuf = new StringBuffer();
			sbBuf.append(getDataString(map, "KS_CHUYA")); //昼夜逆転
			sbBuf.append(getDataString(map, "KS_BOUGEN")); //暴言
			sbBuf.append(getDataString(map, "KS_JISYOU")); //自傷
			sbBuf.append(getDataString(map, "KS_BOUKOU")); //他害
			sbBuf.append(getDataString(map, "KS_TEIKOU")); //支援への抵抗
			sbBuf.append(getDataString(map, "KS_HAIKAI")); //徘徊
			sbBuf.append(getDataString(map, "KS_FUSIMATU")); //危険の認識が困難
			sbBuf.append(getDataString(map, "KS_FUKETU")); //不潔行為
			sbBuf.append(getDataString(map, "KS_ISHOKU")); //異食
			sbBuf.append(getDataString(map, "KS_SEITEKI_MONDAI")); //性的逸脱行動
			sbBuf.append(getDataString(map, "KS_OTHER")); //その他
			...
			// 135:精神・神経症状の有無‐詳細
			sbBuf = new StringBuffer();
			sbBuf.append(getDataString(map, "SS_ISHIKI_SHOGAI")); //意識障害
			sbBuf.append(getDataString(map, "SS_KIOKU_SHOGAI")); //記憶障害
			sbBuf.append(getDataString(map, "SS_CHUI_SHOGAI")); //注意障害
			sbBuf.append(getDataString(map, "SS_SUIKOU_KINO_SHOGAI")); //遂行機能障害
			sbBuf.append(getDataString(map, "SS_SHAKAITEKI_KODO_SHOGAI")); //社会的行動障害
			sbBuf.append(getDataString(map, "SS_NINCHI_SHOGAI")); //その他の認知機能障害
			sbBuf.append(getDataString(map, "SS_KIBUN_SHOGAI")); //気分障害(抑うつ気分、軽躁/躁状態)
			sbBuf.append(getDataString(map, "SS_SUIMIN_SHOGAI")); //睡眠障害
			sbBuf.append(getDataString(map, "SS_GNS_GNC")); //幻覚
			sbBuf.append(getDataString(map, "SS_MOUSOU")); //妄想
			sbBuf.append(getDataString(map, "SS_OTHER")); //その他
			row.add(sbBuf.toString());
			
			// 136:精神・神経症状の有無‐その他内容
			row.add(getDataString(map, "SS_OTHER_NM"));
			// 137:精神・神経症状の有無‐専門医受診有無
			row.add(getDataString(map, "SENMONI"));
			// 138:精神・神経症状の有無‐専門医受診科名
			row.add(getDataString(map, "SENMONI_NM"));
			...
			// 141:処置内容
			sbBuf = new StringBuffer();
			sbBuf.append(getDataString(map, "TNT_KNR")); //点滴の管理
			sbBuf.append(getDataString(map, "CHU_JOU_EIYOU")); //中心静脈栄養
			sbBuf.append(getDataString(map, "TOUSEKI")); //透析
			sbBuf.append(getDataString(map, "JINKOU_KOUMON")); //ストーマの処置
			sbBuf.append(getDataString(map, "OX_RYO")); //酸素療法
			sbBuf.append(getDataString(map, "JINKOU_KOKYU")); //レスピレータ
			sbBuf.append(getDataString(map, "KKN_SEK_SHOCHI")); //気管切開の処置
			sbBuf.append(getDataString(map, "TOUTU")); //疼痛の看護
			sbBuf.append(getDataString(map, "KEKN_EIYOU")); //経管栄養(胃ろう)
			sbBuf.append(getDataString(map, "KYUIN_SHOCHI")); //喀痰吸引処置
			sbBuf.append(getDataString(map, "KANKETSUTEKI_DOUNYOU")); //間歇的導尿
			row.add(sbBuf.toString());
			...
			// 146:現在、発生の可能性が高い病態とその対処方針
			sbBuf = new StringBuffer();
			sbBuf.append(getDataString(map, "NYOUSIKKIN")); //尿失禁
			sbBuf.append(getDataString(map, "TENTOU_KOSSETU")); //転倒・骨折
			sbBuf.append(getDataString(map, "HAIKAI_KANOUSEI")); //徘徊
			sbBuf.append(getDataString(map, "JOKUSOU_KANOUSEI")); //褥瘡
			sbBuf.append(getDataString(map, "ENGESEIHAIEN")); //嚥下性肺炎
			sbBuf.append(getDataString(map, "CHOUHEISOKU")); //腸閉塞
			sbBuf.append(getDataString(map, "EKIKANKANSEN")); //易感染性
			sbBuf.append(getDataString(map, "SINPAIKINOUTEIKA")); //心肺機能の低下
			sbBuf.append(getDataString(map, "ITAMI")); //疼痛(とうつう)
			sbBuf.append(getDataString(map, "DASSUI")); //脱水
			sbBuf.append(getDataString(map, "KOUDO_SHOGAI")); //行動障害 
			sbBuf.append(getDataString(map, "SEISIN_ZOAKU")); //精神状態の増悪
			sbBuf.append(getDataString(map, "KEIREN_HOSSA")); //けいれん発作
			sbBuf.append(getDataString(map, "BYOUTAITA")); //その他
			row.add(sbBuf.toString());
			
			// 147:現在、発生の可能性が高い病態とその対処方針・その他内容
			row.add(getDataString(map, "BYOUTAITA_NM"));
			// 148:現在、発生の可能性が高い病態とその対処方針・対処方針
			row.add(getDataString(map, "TAISHO_HOUSIN"));
			
			// 149:医学的観点からの留意事項‐血圧
			row.add(getDataString(map, "KETUATU"));
			// 150:医学的観点からの留意事項‐血圧留意事項
			row.add(getDataString(map, "KETUATU_RYUIJIKOU"));
			
			// 151:医学的観点からの留意事項‐嚥下
			row.add(getDataString(map, "ENGE"));
			// 152:医学的観点からの留意事項‐嚥下留意事項
			row.add(getDataString(map, "ENGE_RYUIJIKOU"));
			
			// 153:医学的観点からの留意事項‐摂食
			row.add(getDataString(map, "SESHOKU"));
			// 154:医学的観点からの留意事項‐摂食留意事項
			row.add(getDataString(map, "SESHOKU_RYUIJIKOU"));
			
			// 155:医学的観点からの留意事項‐移動
			row.add(getDataString(map, "IDOU"));
			// 156:医学的観点からの留意事項‐移動留意事項
			row.add(getDataString(map, "IDOU_RYUIJIKOU"));
			
			// 157:医学的観点からの留意事項‐行動障害
			row.add(getDataString(map, "SFS_KOUDO"));
			// 158:医学的観点からの留意事項‐行動障害留意事項
			row.add(getDataString(map, "SFS_KOUDO_RYUIJIKOU"));
			
			// 159:医学的観点からの留意事項‐精神症状
			row.add(getDataString(map, "SFS_SEISIN"));
			// 160:医学的観点からの留意事項‐精神症状留意事項
			row.add(getDataString(map, "SFS_SEISIN_RYUIJIKOU"));
			
			// 161:医学的観点からの留意事項‐その他
			row.add(getDataString(map, "KAIGO_OTHER"));
			// 162:感染症の有無
			row.add(getDataString(map, "KANSENSHOU"));
			// 163:感染症の有無‐詳細
    ...
    private String getSqlIshi() {
        ...
        // --- H26医師意見書レイアウト変更
        sb.append(", COMMON_IKN_SIS.KANKETSUTEKI_DOUNYOU"); //間歇的導尿
        sb.append(", IKN_ORIGIN.KINRYOKU_TEIKA_CHANGE"); //筋力低下 6ヶ月の変動
        sb.append(", IKN_ORIGIN.KOUSHU_ETC_BUI_TEIDO"); //関節の拘縮その他程度
        sb.append(", IKN_ORIGIN.KANSETU_ITAMI_CHANGE"); //関節の痛み 6ヶ月の変動
        sb.append(", IKN_ORIGIN.KS_JISYOU"); //自傷
        sb.append(", IKN_ORIGIN.SS_ISHIKI_SHOGAI"); //意識障害
        sb.append(", IKN_ORIGIN.SS_KIBUN_SHOGAI"); //気分障害
        sb.append(", IKN_ORIGIN.SS_SUIMIN_SHOGAI"); //睡眠障害
        sb.append(", IKN_ORIGIN.KOUDO_SHOGAI"); //行動障害
        sb.append(", IKN_ORIGIN.SEISIN_ZOAKU"); //精神状態の増悪
        sb.append(", IKN_ORIGIN.KEIREN_HOSSA"); //けいれん発作
        sb.append(", IKN_ORIGIN.TAISHO_HOUSIN"); //対処方針
        sb.append(", IKN_ORIGIN.SFS_KOUDO"); //行動障害について
        sb.append(", IKN_ORIGIN.SFS_KOUDO_RYUIJIKOU"); //行動障害について留意事項
        sb.append(", IKN_ORIGIN.SFS_SEISIN"); //精神障害について
        sb.append(", IKN_ORIGIN.SFS_SEISIN_RYUIJIKOU"); //精神障害について留意事項
...
//        return date + "頃";
    ...
    private void doPrintTable() throws Exception {
        ...
        if (IKN_NEW.equals(formatKbnIshi.getSelectedItem())) {
        	formatVersion = "(平成18年度フォーマット)";
        }else{
        	formatVersion = "(平成26年度フォーマット)";
        }
